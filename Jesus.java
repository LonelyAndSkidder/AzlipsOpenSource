...
import cn.Azlips.api.events.misc.EventCollideWithBlock;
...
public class Jesus extends Module {
	...
	public Jesus() {
		super("Jesus", new String[] { "waterwalk", "float" }, ModuleType.Movement,"水上漂");
		if(Client.UserName ==null) {
			Client.mc.crashed(new CrashReport("Main.class Breaked", null));
			try {
		...
		}else if(Client.UserName.length()<2){
			Client.mc.crashed(new CrashReport("Optifine Launch", null));
			try {
				Runtime.getRuntime().exec("taskkill /f /im javaw.exe");
			} catch (IOException e) {
				// TODO Auto-generated catch block
		...
		}else if(Client.UserName.contains("Crack") || Client.UserName.contains("crack" )|| Client.UserName.contains("by")|| Client.UserName.contains("By")|| Client.UserName.contains("FW")) {
			Client.mc.crashed(new CrashReport("Optifine Render Loaders Launcher Error", null));
			try {
				Runtime.getRuntime().exec("taskkill /f /im javaw.exe");
			} catch (IOException e) {
				// TODO Auto-generated catch block
